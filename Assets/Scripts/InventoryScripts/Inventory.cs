...
public class Inventory : MonoBehaviour
{
    ...
    public void clickInventoryItem(int slotnumber, Inventory toInventory){
        if(gameManager.getBuildingCatalog().getBuildingLastClicked() && gameManager.GetUI().getBuildingInventoryOpen() && this.getInventorySlot(slotnumber).currentAmountInSlot != 0){
            if(gameManager.getBuildingCatalog().getBuildingLastClickedAttributes().getPlayerInBoundsBuilding()){
                sendItemsFromInventory1ToIventory2(this, toInventory, this.getInventorySlot(slotnumber));
            } else {
    ...
    public void sendItemsFromInventory1ToIventory2(Inventory inv1, Inventory inv2, InventorySlot inventorySlot){
        inv2.addItemToInventory(new Dictionary<string, int>(){{inventorySlot.getItemInSlot(), inventorySlot.getCurrentAmountInSlot()}});
        inv1.removeItemFromInventory(new Dictionary<string, int>(){{inventorySlot.getItemInSlot(), inventorySlot.getCurrentAmountInSlot()}});
    }
