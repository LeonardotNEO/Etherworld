...
public class Healthbar : MonoBehaviour
{
    ...
    void Start()
    {
        startHealth = this.transform.GetComponentInParent<EnemyAttributes>().getMaxHealth();
    }
    void Update()
    {
    ...
    public void updateHealthBar(){
        if(transform.gameObject.activeSelf){
            if(this.transform.GetComponentInParent<EnemyAttributes>().getMaxHealth() != 0 && startHealth == 0){
                startHealth = this.transform.GetComponentInParent<EnemyAttributes>().getMaxHealth();
            }
